...
block_blob_service = BlockBlobService(account_name='storageaccountname', account_key='xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
# Creating the timestamp for blob-container name
date_string = str(now.month) + str(now.day) + str(now.hour) + str(now.minute)
container_name = 'storagecontainername' + date_string
